  ...
  case "$1" in
    ...
    "edp_only")
      swaymsg "output eDP-1 enable" \
             && swaymsg "output eDP-1 resolution 2880x1800 position 0 0 scale 2" \
             && swaymsg "output $external_output disable"
      ;;
    "both")
      swaymsg "output eDP-1 enable" \
             && swaymsg "output eDP-1 resolution 2880x1800 position 1920 682 scale 2" \
             && swaymsg "output $external_output enable primary" \
             && swaymsg "output $external_output resolution 1920x1080 position 0 0"
      ;;
    *)
